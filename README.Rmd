...
```
The sequence1.xml has been edited such that the property 'Maket.Scene.CellDimensions.x' now has the specified property arguments, '1.5;3;4.5;6". You will also see the directory daRtInput/<datettime> has been created and within this directory is sequence files, one for each combination of propertyArgs. Within each sequence file is a script file (shell or batch depending on operting system), which is capable of running the sequence file through dart-maket.
